...
const [profile, setProfile] = useState<ProfileData>({
 ...
 const handleSaveProfile = async (profile: any) => {
  try {
    ...
    // 🔥 1. UPDATE USERS TABLE (MAIN SOURCE)
    const { error: userError } = await supabase
  .from("users")
  .update({
    name: profile.name || "",
    phone: profile.phone || "", // 🔥 FIX
...
console.log("PROVIDER UPDATE:", providerData);
console.log("PROVIDER ERROR:", providerError);
  ...
  <select
    ...
    className="w-full px-4 py-3 rounded-xl glass dark:glass-dark border-2 border-transparent focus:border-purple-500 outline-none text-sm"
  >
