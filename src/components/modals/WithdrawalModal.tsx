...
console.log("UPI ID:", upiId);
    ...
    const { error } = await api.post('provider_request_withdrawal', {
    ...
    console.log("WITHDRAWAL REQUEST:", { amount: withdrawAmount, method: selectedMethod, payout_details: selectedMethod === 'upi' ? { upi_id: upiId } : { bank_name: bankDetails.bankName, account_holder: bankDetails.accountHolderName, account_number: bankDetails.accountNumber, ifsc: bankDetails.ifscCode } });
  ...
  {bankDetails?.bank_name || "No Bank"}
                    ...
                    <div className="pl-13 text-xs text-gray-600 dark:text-gray-400">
                     A/C: {bankDetails?.account_number} • {bankDetails?.ifsc_code}
                    </div>
                  </button>
...
<p>{bankDetails?.bank_name}</p>
  ...
  disabled={loading}
  className="flex-1 py-3 rounded-xl bg-gradient-to-r from-green-500 to-emerald-600 text-white hover:shadow-xl transition-all disabled:opacity-50"
>
