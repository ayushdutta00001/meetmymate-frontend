...
export function BlindDatePaymentNew({ onNavigate, onBack }: BlindDatePaymentNewProps) {
 ...
 useEffect(() => {
  const saved = sessionStorage.getItem('blindDatePayment');
...
useEffect(() => {
 ...
 const handlePayment = async () => {
  ...
  try {
    ...
    console.log(
      "BLIND DATE BOOKING TOTAL:",
      amountInRupees
    );

    console.log(
      "BLIND DATE RAZORPAY AMOUNT IN PAISE:",
      amountInPaise
    );
    ...
    body: {
      ...
      razorpay_signature:
        response.razorpay_signature,
          ...
          console.log(
            "BLIND DATE PAYMENT UPDATE RESULT:",
            paymentResult
          );

          if (!paymentResult?.success) {
            console.error(
              "Blind Date booking payment update failed:",
              paymentResult
            );

            alert(
              "Payment was successful, but we could not update your booking. Please contact support."
            );
          ...
          <div className="flex items-center gap-4">
           <BackButton onClick={onBack} />
            <div>
              ...
              <p className="text-sm text-gray-600 dark:text-gray-400">Review and complete your booking</p>
            </div>
          </div>
        </div>
      </motion.div>
...
>
  <h4 className="mb-4">Blind Date Booking</h4>
...
>
  {processing ? (
    ...
    `Pay & Confirm ₹${bookingData.amount}`
  )}
</button>
          </>
        )}
