...
import { ExperiencedPaymentScreen } from './ExperiencedPaymentScreen';
import { ExperiencedBookingConfirmationScreen } from './ExperiencedBookingConfirmationScreen';
...
type ViewMode = 'entry' | 'customer-discovery' | 'slot-booking' | 'expert-signup' | 'expert-dashboard' | 'expert-access' | 'payment' | 'booking-confirmation';
...
export function FindExperiencedScreen({ onNavigate, onBack, hasBusinessMeetupSubscription = false }: FindExperiencedScreenProps) {
  ...
  return (
    <AnimatePresence mode="wait">
      ...
      {viewMode === 'payment' && selectedExpert && selectedSlot && (
        <ExperiencedPaymentScreen
          onBack={() => setViewMode('slot-booking')}
          onNavigate={(page) => {
            if (page === 'experienced-booking-confirmation') {
              setViewMode('booking-confirmation');
            } else {
              onNavigate(page);
            }
          }}
      ...
      {viewMode === 'booking-confirmation' && (
        <ExperiencedBookingConfirmationScreen
          onNavigate={(page) => {
            if (page === 'home') {
              onNavigate('home');
